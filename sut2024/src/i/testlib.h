...
#define VERSION "0.9.40-SNAPSHOT-ModifiedForICPC"
 ...
 * Kian Mirjalali:
 *
 * Modified to be compatible with DOMJudge & requirements for preparing ICPC tasks
 *
 * * Defined FOR_LINUX in order to force linux-based line endings in validators.
 *
 * * Changed the ordering of checker arguments
 *    from: input_file output_file judge_ans
 *    to: input_file judge_ans feedback_dir < output_file
 *
 * * Changed checker quit behaviors to make it compliant with DOMJudge.
 *
 * * The checker exit codes should always be 42/43 in DOMJudge.
 *
...
NORETURN void InStream::quit(TResult result, const char *msg) {
    ...
    if (testlibMode == _checker) {
        std::string jmPath = feedback_dir+"judgemessage.txt";
        const char* jmPath_cstr = jmPath.c_str();
        std::string printableMessage = __testlib_toPrintableMessage(message);
        const char* printableMessage_cstr = printableMessage.c_str();
        FILE* jmFile = std::fopen(jmPath_cstr, "w");
        if (jmFile == NULL) {
		    std::fprintf(stderr, "Failed to open '%s' for writing. The message was: %s", jmPath_cstr, printableMessage_cstr);
            result = _fail;
        } else {
            std::fprintf(jmFile, "%s", printableMessage_cstr);
            std::fclose(jmFile);
        }
        inf.close();
        ouf.close();
...
NORETURN void __testlib_help() {
    ...
    std::fprintf(stderr, "    [--testset testset] [--group group] input_file judge_ans feedback_dir < output_file\n\n");
...
void registerTestlibCmd(int argc, char *argv[]) {
    ...
    if (argc < 4 || argc > 4) {
        quit(_fail, std::string("Program must be run with the following arguments: ") +
                    std::string("[--testset testset] [--group group] input_file judge_ans feedback_dir < output_file") +
                    "\nUse \"--help\" to get help information");
    }
...
void registerTestlib(int argc, ...) {
    if (argc < 4 || argc > 4)
        quit(_fail, std::string("Program must be run with the following arguments: ") +
                    "input_file judge_ans feedback_dir < output_file");
